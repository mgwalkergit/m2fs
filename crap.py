...
if plot_cmdmap:
    ...
    m2fs_names=np.array([['for','fornax_1','Fornax']],dtype='U100')
...
center=SkyCoord(dsph[1].data['ra'][21],dsph[1].data['dec'][21],unit=(u.deg,u.deg))
...
walker_ra=walker1['RAJ2000']
walker_dec=walker1['DEJ2000']
walker_coords=SkyCoord(walker_ra,walker_dec,unit=(u.deg,u.deg))
walker_xi,walker_eta=mycode.etaxiarr(walker_coords.ra.rad,walker_coords.dec.rad,center.ra.rad,center.dec.rad)
walker_r=np.sqrt(walker_xi**2+walker_eta**2)

walker_mem=np.where((walker_r<60.)&(np.abs(walker_vlos-55.)<50.))[0]
walker_non=np.where((walker_r<60.)&(np.abs(walker_vlos-55.)>=50.))[0]

data=fits.open('m2fs_HiRes_catalog.fits')[1].data
vmean=55.
keep0=np.where((data.good_obs==1)&(data.good_n_obs>0)&(data.target_system=='for')&(data.logg_mean<3.5)&(np.abs(data.vlos_mean-vmean)<50.))[0]
keep=np.where((data.good_obs==1)&(data.good_n_obs>1)&(data.target_system=='for')&(data.logg_mean<3.5)&(np.abs(data.vlos_mean-vmean)<50.))[0]
keep_mr=np.where((data.good_obs==1)&(data.good_n_obs>0)&(data.target_system=='for')&(data.logg_mean<3.5)&(np.abs(data.vlos_mean-vmean)<50.)&(data.feh_mean>-1.5))[0]
keep_mp=np.where((data.good_obs==1)&(data.good_n_obs>0)&(data.target_system=='for')&(data.logg_mean<3.5)&(np.abs(data.vlos_mean-vmean)<50.)&(data.feh_mean<-1.5))[0]
keep2=np.where((data.good_obs==1)&(data.good_n_obs>1)&(data.target_system=='for')&(data.logg_mean<3.5)&(np.abs(data.vlos_mean-vmean)<50.)&(data.vlos_variable_flag))[0]

#plt.scatter(data.vlos_mean[keep0],data.feh_mean[keep0],s=1,alpha=0.3,rasterized=True)
#plt.show()
plt.close()
...
xi,eta=mycode.etaxiarr(coords.ra.rad,coords.dec.rad,center.ra.rad,center.dec.rad)
r=np.sqrt(xi**2+eta**2)
ratio=data.vlos_mean_scatter/data.vlos_mean_error
...
#ell=Ellipse(xy=[field_center_xi/60.,field_center_eta/60],height=axis_major,width=axis_minor,angle=0,fc='0.7',ec=None,fill=True,alpha=1)
axis_major=2*2.*dsph[1].data['rhalf'][21]/np.sqrt(1.-dsph[1].data['ellipticity'][21])/60.#twice Rhalf
axis_minor=axis_major*(1.-dsph[1].data['ellipticity'][21])
ell=Ellipse(xy=[0,0],height=axis_major,width=axis_minor,angle=-dsph[1].data['PA'][21],fc=None,ec='k',fill=False,linestyle='--',lw=0.5)
ax1.add_artist(ell)


#ax1.scatter(walker_xi[walker_non]/60,walker_eta[walker_non]/60,s=1,alpha=0.5,color='k',rasterized=True)
ax1.scatter(walker_xi[walker_mem]/60.,walker_eta[walker_mem]/60.,s=1,alpha=0.3,color='0.5',rasterized=True,label='[Fe/H] unavailable')
#ax1.scatter(xi[keep0]/60,eta[keep0]/60,s=1,alpha=0.5,color='k',rasterized=True)
ax1.scatter(xi[keep_mr]/60,eta[keep_mr]/60,s=1,alpha=0.3,color='r',rasterized=True,label='[Fe/H]$\geq$ -1.5')
ax1.scatter(xi[keep_mp]/60,eta[keep_mp]/60,s=1,alpha=0.5,color='b',rasterized=True,label='[Fe/H]<-1.5')

axis_major=25./60.
axis_minor=25./60.
ell=Ellipse(xy=[-0.05,-0.03],height=axis_major,width=axis_minor,angle=0,fc=None,ec='k',fill=False,alpha=1,linestyle='-',lw=1)
ax1.add_artist(ell)

ell=Ellipse(xy=[0.25,0.25],height=axis_major,width=axis_minor,angle=0,fc=None,ec='k',fill=False,alpha=1,linestyle='-',lw=1)
ax1.add_artist(ell)
ell=Ellipse(xy=[0.35,0.6],height=axis_major,width=axis_minor,angle=0,fc=None,ec='k',fill=False,alpha=1,linestyle='-',lw=1)
ax1.add_artist(ell)
...
ax1.set_yticklabels(['-1','0','1'],fontsize=10)
ax1.legend(loc=3,fontsize=5)
ax1.set_title('Fornax spectroscopic members',fontsize=9)

xxx=scipy.stats.binned_statistic(r[keep0]/60,data.feh[keep0],statistic='mean',bins=5)
yyy=scipy.stats.binned_statistic(r[keep0]/60,data.feh[keep0],statistic='std',bins=5)
ax2.errorbar(xxx.bin_edges[1:],xxx.statistic,yerr=yyy.statistic,fmt='.',color='k')
ax2.set_xlim([0,1.3])
ax2.set_xlabel('R [deg]')
ax2.set_ylabel(r'$\overline{\mathrm{[Fe/H]}}$')
ax2.set_title('Fornax metallicity gradient',fontsize=9)
plt.savefig('noirlab_fig1.pdf',dpi=300)
plt.show()
plt.close()
