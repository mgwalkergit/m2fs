...
if get_surveyphot:
    ...
    for i in range(0,len(fitsfile)):
        ...
        survey_phot=m2fs.survey_phot(gaia=gaia,des=des,decals=decals,sdss=sdss,ps1=ps1)
...
if make_skysub:
    ...
    for i in range(0,len(fitsfile)):
        ...
        for j in targets:
            ...
            if ((len(np.where(fitsobject.mask[j]==False)[0])>0)&(np.max(continuum>0.)>0.)):#write skysub.dat file only if >100 good pixels in spectrum
...
#                    if fitsobject.mask[j][k]==False:
                    ...
                    #print(k,spec2[k],fitsobject.mask[j][k])
                    if spec2[k]!=spec2[k]:
                        if fitsobject.mask[j][k]==False:
                            np.pause()
                    g1.write(string)
...
#                    postfit_temperature.append(postfit[6].data['temperature'][j])
                    ...
                    postfit_wav_npoints.append(str(postfit[6].data['wav_npoints'][j].tolist()))
                    postfit_wav_rms.append(str(postfit[6].data['wav_rms'][j]).replace('[','').replace(']',''))
                    postfit_wav_resolution.append(str(postfit[6].data['wav_resolution'][j]).replace('[','').replace(']',''))
                    postfit_wav_min.append(str(postfit[6].data['wav_min'][j]).replace('[','').replace(']',''))
                    postfit_wav_max.append(str(postfit[6].data['wav_max'][j]).replace('[','').replace(']',''))
                    postfit_row.append(postfit[6].data['row'][j])
                    postfit_temperature.append(str(postfit[6].data['temperature'][j]).replace('[','').replace(']',''))
...
#        col139=fits.Column(name='temperature',format='PD()',array=postfit_temperature[targets],unit='degree Celcius')
        col133=fits.Column(name='wav_npoints',format='A30',array=postfit_wav_npoints[targets],unit='')
        col134=fits.Column(name='wav_rms',format='A30',array=postfit_wav_rms[targets],unit='Angstrom')
        col135=fits.Column(name='wav_resolution',format='A30',array=postfit_wav_resolution[targets],unit='Angstrom')
        col136=fits.Column(name='wav_min',format='A30',array=postfit_wav_min[targets],unit='Angstrom')
        col137=fits.Column(name='wav_max',format='A30',array=postfit_wav_max[targets],unit='Angstrom')
        col139=fits.Column(name='temperature',format='A30',array=postfit_temperature[targets],unit='degree Celcius')
...
#            catalog[thing[k]+'_error']=10.**bestfit['parameters'][0]+10.**bestfit['parameters'][1]*catalog_raw[1].data[thing[k]+'_raw_error']
        ...
        for i in range(0,len(catalog)):
            nthar[i]=len(catalog['wav_rms'][i].split())
            temp_min[i]=np.min(np.asarray(catalog['temperature'][i].split(),dtype='float'))
            temp_max[i]=np.max(np.asarray(catalog['temperature'][i].split(),dtype='float'))
